...
func TransferBFC(handler *sql.DB, airdropAmount float64, rpcUrl string) {
	data, err := util.QueryTransactions(handler)
	if err != nil {
		fmt.Printf("QueryTransactions failed, error is %s\n", err.Error())
		return
	}
	fmt.Println(data)

	for _, v := range data {
		f := TransferBFCOnChain(v.Address, airdropAmount, rpcUrl)
		if f {
			util.UpdateData(handler, v.TxDigest)
		} else {
			fmt.Printf("transfer failed, params is %s %f %s\n", v.Address, airdropAmount, rpcUrl)
		}
	}
}

func TransferBFCOnChain(data string, airdropAmount float64, rpcUrl string) bool {
	hexAddress := util.ConvertBfcAddressToHexAddressWithCheck(data)
...
func GetAddressBalance(Address string, rpcUrl string) {
	c, _ := client.Dial(rpcUrl)

	//hexAddress := util.ConvertBfcAddressToHexAddress(Address)
	balanceAddress, _ := bfc_types.NewAddressFromHex(Address)
	balance, err := c.GetBalance(context.Background(), *balanceAddress, types.BFC_COIN_TYPE)

	if err != nil {
		fmt.Printf("get balance failed, error is %s\n", err.Error())
	}
	humanBalance, _ := balance.TotalBalance.Float64()
	humanBalance = humanBalance / 1e9
	fmt.Printf("balance is %.2f bfc\n", humanBalance)
