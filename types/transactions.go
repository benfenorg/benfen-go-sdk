...
type BfcDaoInfoResponseConfig struct {
	VotingDelay       uint64 `json:"voting_delay"`
	VotingPeriod      uint64 `json:"voting_period"`
	VotingQuorum      uint64 `json:"voting_quorum_rate"`
	VotingActionDelay uint64 `json:"min_action_delay"`
}
type BfcProposalRecord struct {
}
type BfcVotingRecord struct {
}
type BfcVotingPool struct {
	BfcBalance       uint64 `json:"bfc_balance"`
	PoolTokenBalance uint64 `json:"pool_token_balance"`
}
type BfcDaoInfoResponse struct {
	Admin          bfc_types.BfcAddress     `json:"admin"`
	Config         BfcDaoInfoResponseConfig `json:"config"`
	ProposalRecord []BfcProposalRecord      `json:"proposal_record"`
	VotingRecord   []BfcVotingRecord        `json:"voting_record"`
	VotingPool     BfcVotingPool            `json:"voting_pool"`
}
