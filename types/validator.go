...
type BfcValidatorSummary struct {
	BfcAddress             bfc_types.BfcAddress `json:"suiAddress"`
	...
	//stable pool
	StablePools stablePools `json:"stablePools"`
}
type stablePools struct {
	StablePoolsId StablePoolId          `json:"id"`
	Size          SafeBfcBigInt[uint64] `json:"size"`
}
type StablePoolId struct {
	Sid string `json:"id"`
}
